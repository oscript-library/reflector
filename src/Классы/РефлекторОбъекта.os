...
//             * Значение - строка - значение параметра аннотации
//
Функция ПолучитьТаблицуМетодовПоАннотации(АннотацияФильтр) Экспорт

	Если Сброс Тогда
		ПрочитатьТаблицыОбъекта();
	КонецЕсли;

	ОтфильтрованнаяТаблица = ТаблицаМетодов.СкопироватьКолонки();
	...
	Для Каждого Метод Из ТаблицаМетодов Цикл
		...
		Для Каждого Аннотация Из Метод.Аннотации Цикл
			...
			Если НРег(Аннотация.Имя) = Нрег(АннотацияФильтр) Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Метод);
			КонецЕсли;	
