...
//             * Значение - строка - значение параметра аннотации
//
Функция ПолучитьТаблицуМетодов(АннотацияФильтр = Неопределено) Экспорт
	...
	Если АннотацияФильтр = Неопределено Тогда
		Возврат ТаблицаМетодов.Скопировать();
	Иначе
		Возврат ФильтроватьТаблицуПоАннотациям(ТаблицаМетодов, АннотацияФильтр);
	КонецЕсли;
...
Процедура ОбработатьТаблицаМетодов(ТаблицаМетодовОбъекта)
	...
	ОбработатьПараметры = НЕ ТаблицаМетодовОбъекта.Колонки.Найти("КоличествоПараметров") = Неопределено;
	Для Каждого СтрокаТаблицы Из ТаблицаМетодовОбъекта Цикл
		...
		ПривестиТаблицуАннотацийЕдинообразию(СтрокаТаблицы.Аннотации);

	КонецЦикла;

КонецПроцедуры

Процедура ПривестиТаблицуАннотацийЕдинообразию(ТаблицаАннотаций)
...
Функция ФильтроватьТаблицуПоАннотациям(ТаблицыДанных, АннотацияФильтр)
	...
	Для Каждого СтрокаТаблицы Из ТаблицыДанных Цикл
		...
		Для Каждого Аннотация Из СтрокаТаблицы.Аннотации Цикл
			...
			Если Аннотация.Имя = Нрег(АннотацияФильтр) Тогда
